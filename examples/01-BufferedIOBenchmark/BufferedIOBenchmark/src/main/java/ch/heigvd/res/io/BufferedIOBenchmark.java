...
public class BufferedIOBenchmark {
    ...
    private long produceTestData(IOStrategy ioStrategy, long numberOfBytesToWrite, int blockSize) {
        ...
        long t = Timer.takeTime();

        LOG.log(Level.INFO, "  > Done in {0} ms.", t);
     ...
     */
    private static String formatLog(String operation, IOStrategy ioStrategy, long blockSize, long fileSizeInBytes, long durationInMs) {
        return operation + "," + ioStrategy.toString() + "," + String.valueOf(blockSize) + "," + String.valueOf(fileSizeInBytes) + "," + String.valueOf(durationInMs) + "\n";
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // Writers
            FileWriter fw = new FileWriter(outf.getName(), true);
            BufferedWriter bw = new BufferedWriter(fw);

            // Write header
            bw.write("operation,strategy,blockSize,fileSizeInBytes,durationInMs\n");

            LOG.log(Level.INFO, "");
            LOG.log(Level.INFO, "*** BENCHMARKING WRITE OPERATIONS (with BufferedStream)", Timer.takeTime());
            t = bm.produceTestData(IOStrategy.BlockByBlockWithBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 500);
            bw.write(formatLog("WRITE", IOStrategy.BlockByBlockWithBufferedStream, 500, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.produceTestData(IOStrategy.BlockByBlockWithBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 50);
            bw.write(formatLog("WRITE", IOStrategy.BlockByBlockWithBufferedStream, 50, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.produceTestData(IOStrategy.BlockByBlockWithBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 5);
            bw.write(formatLog("WRITE", IOStrategy.BlockByBlockWithBufferedStream, 5, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.produceTestData(IOStrategy.ByteByByteWithBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 0);
            bw.write(formatLog("WRITE", IOStrategy.ByteByByteWithBufferedStream, 0, NUMBER_OF_BYTES_TO_WRITE, t));


            LOG.log(Level.INFO, "");
            LOG.log(Level.INFO, "*** BENCHMARKING WRITE OPERATIONS (without BufferedStream)", Timer.takeTime());
            t = bm.produceTestData(IOStrategy.BlockByBlockWithoutBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 500);
            bw.write(formatLog("WRITE", IOStrategy.BlockByBlockWithoutBufferedStream, 500, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.produceTestData(IOStrategy.BlockByBlockWithoutBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 50);
            bw.write(formatLog("WRITE", IOStrategy.BlockByBlockWithoutBufferedStream, 50, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.produceTestData(IOStrategy.BlockByBlockWithoutBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 5);
            bw.write(formatLog("WRITE", IOStrategy.BlockByBlockWithoutBufferedStream, 5, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.produceTestData(IOStrategy.ByteByByteWithoutBufferedStream, NUMBER_OF_BYTES_TO_WRITE, 0);
            bw.write(formatLog("WRITE", IOStrategy.ByteByByteWithoutBufferedStream, 0, NUMBER_OF_BYTES_TO_WRITE, t));


            LOG.log(Level.INFO, "");
            LOG.log(Level.INFO, "*** BENCHMARKING READ OPERATIONS (with BufferedStream)", Timer.takeTime());
            t = bm.consumeTestData(IOStrategy.BlockByBlockWithBufferedStream, 500);
            bw.write(formatLog("READ", IOStrategy.BlockByBlockWithBufferedStream, 500, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.consumeTestData(IOStrategy.BlockByBlockWithBufferedStream, 50);
            bw.write(formatLog("READ", IOStrategy.BlockByBlockWithBufferedStream, 50, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.consumeTestData(IOStrategy.BlockByBlockWithBufferedStream, 5);
            bw.write(formatLog("READ", IOStrategy.BlockByBlockWithBufferedStream, 5, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.consumeTestData(IOStrategy.ByteByByteWithBufferedStream, 0);
            bw.write(formatLog("READ", IOStrategy.ByteByByteWithBufferedStream, 0, NUMBER_OF_BYTES_TO_WRITE, t));


            LOG.log(Level.INFO, "");
            LOG.log(Level.INFO, "*** BENCHMARKING READ OPERATIONS (without BufferedStream)", Timer.takeTime());
            t = bm.consumeTestData(IOStrategy.BlockByBlockWithoutBufferedStream, 500);
            bw.write(formatLog("READ", IOStrategy.BlockByBlockWithoutBufferedStream, 500, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.consumeTestData(IOStrategy.BlockByBlockWithoutBufferedStream, 50);
            bw.write(formatLog("READ", IOStrategy.BlockByBlockWithoutBufferedStream, 50, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.consumeTestData(IOStrategy.BlockByBlockWithoutBufferedStream, 5);
            bw.write(formatLog("READ", IOStrategy.BlockByBlockWithoutBufferedStream, 5, NUMBER_OF_BYTES_TO_WRITE, t));

            t = bm.consumeTestData(IOStrategy.ByteByByteWithoutBufferedStream, 0);
            bw.write(formatLog("READ", IOStrategy.ByteByByteWithoutBufferedStream, 0, NUMBER_OF_BYTES_TO_WRITE, t));
